...
las limitaciones que esas mediciones tienen. La conclusión principal es que el
motor funciona y está medido, pero el corpus disponible no representa el caso de
uso, por lo que la cifra titular queda declarada \pend{} en lugar de publicarse.
\end{abstract}
...
% ===========================================================================
\section{Punto 2 --- Corpus de video \pend}
...
\subsection{Fuentes descartadas}

Se evaluaron otros dos conjuntos de datos disponibles y se descartaron por
razones técnicas:

\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Conjunto} & \textbf{Contenido} & \textbf{¿Sirve para Nivel 1?} \\
\midrule
\texttt{shoplifting2}   & 182 \texttt{.mp4} reales, 30\,fps      & Sí (el usado) \\
\texttt{archive (1)}    & 32.458 PNG, \emph{stride} 10           & Parcial: $\approx$3\,fps efectivos \\
Roboflow COCO           & 1.006 JPG, particiones aleatorias      & \textbf{No} \\
\bottomrule
\end{tabular}
\end{center}

La exportación de Roboflow es inservible para sustracción de fondo: la partición
aleatoria en \texttt{train/valid/test} y el aumento de datos destruyen el orden
temporal, y la sustracción de fondo necesita fotogramas consecutivos.
...
\noindent
Es decir: \dato{el Nivel~2 debe costar más de 19,8\,ms por fotograma} para que la
cascada tenga sentido.
...
2 & Corpus de video     & \pend{} Herramienta completa; los 182 clips
                           disponibles no son interiores comerciales. \\
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Metraje real de un interior comercial.} Es el bloqueo principal.
        Hace falta una cámara fija en una tienda, restaurante, caja o entrada,
        grabando \emph{horas seguidas} —incluyendo los ratos muertos— y no clips
        de 11 segundos elegidos porque en ellos ocurre algo. Sin eso, la cifra
        de descarte no puede publicarse ni usarse para proyectar coste.
  ...
  \item \textbf{Fijar una tarifa de CPU verificada} y convertir los
        ...
        el metraje: puede dejarse parametrizado.

  \item \textbf{Probar el Nivel~1 con poca luz real}, con personas en escena.
        El corpus actual no permite responderlo.
